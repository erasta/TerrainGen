class Tree {
    constructor(pos, size) {
        ...
        var t = this.trunk(size, Math.floor(rand(1, 6)));
        t.position.copy(pos);
        this.mesh.add(t);
    }
    ...
    trunk(size, depth) {
        ...
        if (depth > 0) {
            for (var b = 0; b < branches; ++b) {
                var t = this.trunk(size * 0.8, depth - 1);
                m.add(rotate(t, rand(-20, 20), rand(-20, 20) + 360 / branches * b, rand(-20, 20) + 30));
            }
        } else {
            var ball = new THREE.Mesh(new THREE.SphereGeometry(size * 0.5 * rand(0.5, 2)), Tree.LeafMaterial);
